...
//#define 197.326 197.326
#define MAXPOINT 100

void plot_f1_normalize()
{
	gROOT->Reset();
	gStyle->SetOptStat(0);
...
//	TGraph* gr_c12c12_esw_uf = new TGraph("esw/esw_uf.xsec","%lg %lg");
//	convert_xsec_2_s(gr_c12c12_esw_uf,6.0,1.);
	TGraph* gr_c12c12_esw_uf = new TGraph("esw/c12c12_esw.xsec","%lg %lg");
	TGraph* gr_c12c12_esw_uf2 = new TGraph("esw/c12c12_esw.xsec","%lg %lg");
	
	TGraph* gr_c12c12_esw = new TGraph("esw/c12c12_esw.xsec","%lg %lg");
	convert_xsec_2_s(gr_c12c12_esw,6.0,1.e-3);
	gr_c12c12_esw->SetLineColor(kBlack);
	gr_c12c12_esw->SetLineWidth(2);
	gr_c12c12_esw->Draw("C");

	const Double_t z1ref = 6, z2ref = 6, a1ref = 12, a2ref = 12;
	double z1 = 6, z2 = 6, a1 = 12, a2 = 13;  
	cal_uf(gr_c12c12_esw_uf,z1,a1,z2,a2,"esw/c12c12_esw.xsec",1e-3,"c12c13/cross.dat",z1ref,a1ref,z2ref,a2ref,"c12c12/cross.dat");
	cal_uf_2(gr_c12c12_esw_uf2,z1,a1,z2,a2,"esw/c12c12_esw.xsec",1e-3,"c12c13/cross.dat",z1ref,a1ref,z2ref,a2ref,"c12c12/cross.dat");
	gr_c12c12_esw_uf->SetLineColor(kBlack);
	gr_c12c12_esw_uf2->SetLineColor(kBlue);

	gr_c12c12_esw_uf->SetLineWidth(2);
	gr_c12c12_esw_uf2->SetLineWidth(2);

	//gr_c12c12_esw_uf->Draw("C");
	gr_c12c12_esw_uf2->Draw("C");
	//leg->AddEntry(gr_c12c12, "^{12}C+^{12}C(ESW)", "L");
}
...
void cal_uf_2(TGraph* uf,Double_t z1, Double_t a1, Double_t z2, Double_t a2, char* f2, Double_t f2_factor, char* f1, Double_t  z1ref, Double_t a1ref, Double_t z2ref, Double_t a2ref, char* f0) 
{
	TGraph* FF2=new TGraph(f2);
	TGraph* FF1=new TGraph(f1);
	TGraph* FF0=new TGraph(f0);
...
Double_t ff(Double_t z1, Double_t a1, Double_t z2, Double_t a2,Double_t ecm, Double_t sigma) 
{
	...
	Double_t fk=sqrt(2*mu*931.5*ecm)/197.326;
	// sigma in barn
...
{	
	...
	// get log(sigma*Ecm)
	temp=temp-log(2*mu_ref*931.5/3.1415926/(197.326*197.326)*100.0);
	// convert to Smod
	temp=exp(temp+31.29*z1ref*z2ref*sqrt(mu_ref/(ecm_ref*1000))+0.46*ecm_ref*0);
	//temp=exp(temp+87.21/sqrt(ecm_ref)+0.46*ecm_ref);
//	temp=exp(temp)/ecm_ref;
	//> for 12C12C, par is 87.2537
