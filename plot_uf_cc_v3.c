...
//	leg->SetFillColor(10);
	...
	*/
	...
	if(theory_opt == 1)
	{
		...
		leg->AddEntry(ufc12c12, "^{12}C+^{12}C    M3Y", "l");
		...
		leg->AddEntry(ufc12c13, "^{12}C+^{13}C    M3Y", "l");
		...
		leg->AddEntry(ufc13c13, "^{13}C+^{13}C    M3Y", "l");
	}
	// tdhf
...
void plot_exp_c12c12_data(TLegend *leg)
{
	...
	leg->AddEntry(gr_c12c12_ag, "^{12}C+^{12}C Aguilera(2006)", "p");
...
void plot_exp_c12c13_data(TLegend *leg)
{
	...
	gr_c12c13_no->Draw("P");	
	cal_uf_grerr(6, 12, 6, 13, gr_c12c13_imp, 1.41/1.5, "c12c13/cross.dat", z1ref, a1ref, z2ref, a2ref, "c12c12/cross.dat");
	gr_c12c13_imp->Draw("P");
	leg->AddEntry(gr_c12c13_st, "^{12}C+^{13}C Stokstad(1976)", "p");
	leg->AddEntry(gr_c12c13_imp, "^{12}C+^{13}C IMP(2016)", "p");
...
//	define the reference system
	...
	leg->AddEntry(gr_c13c13_trent, "trent ^{13}C+^{13}C", "p");
}
